...
def compute_features(df, rolling_n=5, top_n=6):
    ...
    if {"CE_openInterest","PE_openInterest","CE_strikePrice"}.issubset(df.columns):
        ...
        top_strikes = (df.groupby("CE_strikePrice")["total_OI"]
    ...
    df[["CE_vol_delta","PE_vol_delta"]] = df[["CE_vol_delta","PE_vol_delta"]].fillna(0)
...
# interpret PCR
def interpret_pcr(p):
    if p < 0.7: return "🐂 Bullish sentiment ‑ fewer puts, more calls."
    if 0.7 <= p <= 1.2: return "🟧 Neutral / range ‑ balanced flow."
    return "🐻 Bearish sentiment ‑ more puts built up."
st.caption(f"**PCR Interpretation:** {interpret_pcr(latest['PCR_OI'])}")

# -------------- COLOR MAPS --------------
def color_signal(val):
    colors = {
        "BUY_CALL":"background-color:#66ff66;",
        "BUY_PUT":"background-color:#339933; color:white;",
        "SELL_STRANGLE":"background-color:#ffa64d;",
        "MOMENTUM_TRADE":"background-color:#00b300; color:white;",
        "EXIT_POSITION":"background-color:#ff4d4d; color:white;",
        "HOLD":"background-color:#ffb380;"
    }
    return colors.get(val, "")

def color_bias(val):
    if val=="bullish": return "background-color:#b3ffb3;"
    if val=="bearish": return "background-color:#ff9999;"
